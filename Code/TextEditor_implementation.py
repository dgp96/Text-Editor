...
elif wx.Platform == '__WXMAC__':					#MAC-
    faces = { 'times': 'Times New Roman',
              'mono' : 'Monaco',
              'helv' : 'Arial',
              'other': 'Comic Sans MS',
...
class MainWindow(wx.Frame):
	def __init__(self, parent, title):
		# variables for file i/o and find, replace
		self.dirname = ''
		...
		#Four empty dictionaries created
		...
		self.control.SetProperty("tab.timmy.whinge.level", "1") #determines how to indicate bad indentation
		...
		#editmenu.AppendSeparator()
		#menuFont = editmenu.Append(wx.ID_FONT, "Fon&t", "Select Font")
		menuFind = editmenu.Append(wx.ID_FIND, "&Find and Replace", "Find and replace word")
		menuSpell = editmenu.Append(wx.ID_ANY, "S&pell Check", "Check Spelling of Current Contents of File")
		...
		self.Bind(wx.EVT_MENU, self.onButton, menuFind)
		self.Bind(wx.EVT_MENU, self.onSpell, menuSpell)
		# self.Bind(wx.EVT_FIND, self.onFind)
		self.Bind(wx.EVT_FIND_NEXT, self.onFindNext)
		self.Bind(wx.EVT_FIND_REPLACE_ALL, self.onFindAndReplaceAll)
		self.Bind(wx.EVT_FIND_REPLACE, self.onFindAndReplace)
	...
	def OnClose(self, e):
		...
		if(file == "Untitled.txt"):
			dlg = wx.MessageDialog(self, "Do you want to save?", "Unsaved Changes!", wx.YES_NO)
			if(dlg.ShowModal() == wx.ID_NO):
				self.OnExitApp(e)
			else:
				self.OnSaveAs(e)
		else:
			f = open(os.path.join(direc,file), 'r')
			text = f.read()
			if(self.control.GetValue() == text):
				self.OnExitApp(e)
			else:
				dlg = wx.MessageDialog(self, "Do you want to save?", "Unsaved Changes!", wx.YES_NO)
				if(dlg.ShowModal() == wx.ID_NO):
	...
	def onButton(self,e):
		self.txt = self.control.GetValue()
		self.data = wx.FindReplaceData()   # initializes and holds search parameters
		self.dlg = wx.FindReplaceDialog(self.control, self.data, 'Find and Replace',wx.FR_REPLACEDIALOG)
		self.dlg.Show()
	...
	# 	self.length = len(fstring)
	# 	if(self.pos != self.prevpos):
	# 		self.count = self.count + 1
		#self.control.SetStyle(self.pos, self.pos+self.length, wx.TextAttr("red", "black"))		
	...
	def onFindNext(self, e):
		...
		self.prevpos = self.pos
		self.length = len(fstring)
		self.pos = self.txt.find(fstring, self.pos+self.length)
		# print self.pos
		if(self.pos == -1):
			self.count = 0
			return
	...
	def onFindAndReplaceAll(self,e):
		self.txt = self.control.GetValue()
		fstring = self.data.GetFindString()
		rstring = self.data.GetReplaceString()
		temp = self.txt
		temp = temp.replace(fstring,rstring)
		self.control.SetValue(temp)

	def onFindAndReplace(self,e):
		self.txt = self.control.GetValue()
		fstring = self.data.GetFindString()
		rstring = self.data.GetReplaceString()
		temp = self.txt
		...
		# print self.count
		if(self.count == 0):	
			temp = temp.replace(fstring,rstring,1)
		else:
			try:
				inds = [i for i in range(len(temp) - len(fstring)+1) if temp[i:i+len(fstring)] == fstring]
				temp = list(temp)
				temp[inds[self.count-1]:inds[self.count - 1]+len(fstring)] = rstring
				temp = ''.join(temp)
			except:
				temp = self.txt
	...
	def onSpell(self, e):
		#Implement here
		fr = self.control.GetValue()
		dlg = wxSpellCheckerDialog(None,-1,"")
		chkr = SpellChecker("en_US",fr)
	...
	def OnAbout(self, e):
		...
		dlg = wx.MessageDialog(self, "An elegant, yet simple, text editor made with Python and wxPython for ease of writing Python codes, though they cannot be run here directly.\nCreated by Dharmit Dosani, Dharmit Prajapati and Kevin Daftary.", "About My Text Editor", wx.OK)
		dlg.ShowModal()
		dlg.Destroy()
...
app = wx.App(False)
frame = MainWindow(None, "TextEditor")
"""fr = open(text_temp,'r')
text=fr.read()
dlg = wxSpellCheckerDialog(None,-1,"")
